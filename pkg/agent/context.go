...
type ContextBuilder struct {
	...
	// It supplies the builder-level default for systemPromptBuildOptions.
	// AllowedSkills; a per-request AllowedSkills takes precedence.
...
// Use ["*"] - or leave empty - to include all skills.
func (cb *ContextBuilder) SetSkillsFilter(filter []string) {
	cb.skillsFilter = filter
	cb.InvalidateCache()
}

// containsSkillWildcard reports whether a skills allowlist contains the "*"
// wildcard, meaning "include every skill". Entries are trimmed to match the
// normalization upstream's cleanAllowedSet applies.
func containsSkillWildcard(allowed []string) bool {
...
func (cb *ContextBuilder) buildSystemPromptParts(opts systemPromptBuildOptions) []PromptPart {
	...
	if opts.IncludeSkillCatalog {
		// Fork: fall back to the builder-level filter (set per tenant via
		// SetSkillsFilter) when the request carries no explicit allowlist.
		allowedSkills := opts.AllowedSkills
		if len(allowedSkills) == 0 {
			allowedSkills = cb.skillsFilter
		}
		// Fork: "*" means "all skills". Upstream's cleanAllowedSet has no
		// wildcard branch, so without this a ["*"] allowlist would match only
		// a skill literally named "*" and yield an empty catalog. A nil
		// allowlist is upstream's own "include everything" path.
		if containsSkillWildcard(allowedSkills) {
			allowedSkills = nil
		}
		skillsSummary = cb.buildSkillsSummary(allowedSkills)
	}
	if skillsSummary != "" {
